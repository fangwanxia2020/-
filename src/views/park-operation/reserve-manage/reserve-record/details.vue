<template>
  <hqit-content-page :title="title">
    <template slot="content">
          ...
          <el-row v-for="(item, idx) of reserveList" :key="idx" class="box">
            ...
            <el-col :span="16" class="box-card">
              <div class="bg">
                <div class="card-top">
                  <div
                    v-if="item.isPrincipal == 1 && type != 'reserve'"
                ...
                <el-col :span="12">
                  ...
                  <el-form-item label="拜访日期：">
                    {{
                      ...
                      visitMsg.visitEndTime &&
                      visitMsg.visitEndTime.split(" ")[0]
                    }}
                  </el-form-item>
                  <el-form-item
                    label="拜访事由："
      ...
      <div class="footer">
        <template v-if="visitMsg.approvalStatus">
          <el-button v-if="isApproval" type="primary" @click="linkTo">
            编辑
          </el-button>
        </template>
      </div>
...
export default {
  components: { ViewImage, ImgUpload },
  props: {
    name: {
      type: String,
      default: "张三",
  ...
  data() {
    return {
      ...
      threeImgList: [], //三证图片
      equipmentTypeList: [], //机具类型
      allMachineKindList: [], //全部机具种类
      carTypeList: [], //全部车辆种类
      dialogImageUrl: "",
      dialogVisible: false,
    };
  ...
  watch: {
    visitMsg(val) {
      ...
      if (val.approvalStatus == 2 || val.approvalStatus == 3) {
  ...
  methods: {
    // 行人详情
    getVisitDetails(id) {
      this.title = `行人预约— 查看— ${this.$route.query.name}`;
    ...
    getCarDetails(id) {
      getCarDetails(id).then(async (res) => {
        ...
        this.title = `人车预约— 查看— ${this.reserveList[0].name}`;
        this.getVisitorImg();
    ...
    getEquipmentDetails(id) {
      getEquipmentDetails(id).then(async (res) => {
        ...
        this.title = `机具预约— 查看— ${this.reserveList[0].name}`;
    ...
    // 访客信息图片转换
    getVisitorImg() {
      this.reserveList.forEach(async (item) => {
        let res1 = await fileBatchDetail(item.facePhotoId.split(","));
        this.$set(item, "facePhotoIdList", this.$getImgArr(res1.data));
        this.$set(item, "facePhotoUrl", item.facePhotoIdList[0].url);
      });
    },
    // 查看图片
    ...
    linkTo() {
      if (
        this.visitMsg.approvalStatus == "2" ||
        this.visitMsg.approvalStatus == "3"
      ) {
