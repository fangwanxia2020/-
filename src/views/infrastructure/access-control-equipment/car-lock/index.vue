  ...
  <hqit-content-page type="list" style="height: calc(100vh - 145px)">
    <template slot="content">
      <hqit-table
        ...
        @reset="handleReset"
      >
        <template slot="isOnline" slot-scope="{ rowData }">
          <span v-if="rowData.isOnline">
...
export default {
  ...
  watch: {
    "dictObj.base_camera_brand": function (val) {
      this.searchConfig.find((v) => v.prop == "cameraBrandId").options =
        val.map((item) => {
  ...
  methods: {
    ...
    handleCheck(row) {
      this.$router.push({
        path: "/gatedpark/infrastructure/access-control-equipment/car-lock-check",
        query: { id: row.id },
      });
    },
    // 修改
    ...
    async handleDel(row) {
      const valid = await this.errorBoundary(
        this.delPrompt(this, row.cameraName),
      );
      if (valid) {
        delCarLock([row.id]).then((res) => {
