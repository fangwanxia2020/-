...
import { getVehicleList, getCarKind } from "@/api/reserve-manage/index.js";
import {
  hasNeedApproval,
  ApprovalTemplate,
  getTemplateId,
...
export default {
  ...
  methods: {
    async getList(params) {
      ...
      let res = await getVehicleList(resParams);
    ...
    getCarKind() {
      ...
      getCarKind(params).then((res) => {
        this.searchConfig.find((v) => v.prop == "vehicleType").options =
    ...
    linkDetails(row) {
      this.$router.push({
        ...
        query: { id: row.id, type: "car", name: row.driverPersonName },
      });
    },
    // 修改
    handleEdit(row) {
    ...
    recall(row) {
      this.$confirm(`是否撤回当前审批`, "提示")
        .then(() => {
          getTemplateId({ domainId: 5, externalBizId: row.id }).then((res) => {
            withdrawTask({
              instanceId: res.data,
            }).then(() => {
