...
export default {
  ...
  computed: {
    visitedViews() {
      ...
      // query加上 changeTagName='' 手动修改顶部面包屑title
      list.map((item) => {
        if (item.query.hasOwnProperty("changeTagName")) {
          console.log(item.title.indexOf(" - ") == -1);
          let title =
            item.title.indexOf(" - ") == -1
              ? item.title
              : item.title.split(" - ")[1];
          item.title = `${item.query.changeTagName} - ${title}`;
          //项目分解的tabs
        } else if (
          (item.query.hasOwnProperty("projectName") &&
            item.query.hasOwnProperty("projectId")) ||
          (item.query.hasOwnProperty("name") &&
            item.query.hasOwnProperty("progressId"))
        ) {
          let title =
            item.title.indexOf("-") == -1
              ? item.title
          ...
          item.title = `${
            item.query.projectName ? item.query.projectName : item.query.name
          }-${title}`;
        }
      });
      return list;
      ...
      if (
        ...
        for (var i = 0; i < this.visitedViews.length; i++) {
          if (this.visitedViews[i].path == this.$route.path) {
            // console.log("子应用", this.visitedViews);
