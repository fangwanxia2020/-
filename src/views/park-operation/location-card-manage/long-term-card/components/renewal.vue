<template>
  <div class="content-box">
    ...
    >
      ...
      <el-row>
        <el-col :span="16">
          <div v-if="isCheck">请选择需要续期的类型</div>
        </el-col>
        <el-col :span="8">
    ...
    <el-tabs v-model="renewalType" class="cl-content-left">
    ...
    <div v-if="renewalType == 'first'" class="cl-content-left">
      <CtTable
        ref="FirstTable"
        :table-height="null"
            ...
            >
              <template slot-scope="scope">
                <span
                  ...
                  >{{ scope.row.applyNum && scope.row.applyNum }}</span
                >
              </template>
            </el-table-column>
          </template>
    ...
    <div v-if="renewalType == 'second'" class="cl-content-left">
      ...
      <el-table :data="PersonDataList" style="width: 100%">
        ...
        <el-table-column label="卡号" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ scope.row.locCardDto && scope.row.locCardDto.cardNo }}
          </template>
        </el-table-column>
    ...
    <div v-if="renewalType == 'third'" class="cl-content-left">
      <div class="cl-charge" style="margin-bottom: 10px">
        <el-button
          v-if="!isCheck"
          type="primary"
    ...
    <el-dialog
      ...
      width="1200px"
      title="批次信息"
      :visible.sync="batchInfo"
    >
      <batch-info :batch-id="batchId" />
      <span slot="footer" class="dialog-footer">
        <el-button class="cancelButton" @click="batchInfo = false">
          取消
        </el-button>
        <el-button
          class="confirmButton"
          type="primary"
          @click="batchInfo = false"
        >
          确定
        </el-button>
      </span>
  ...
  methods: {
      ...
      this.$refs["baseForm"].validate((valid) => {
        if (valid) {
          ...
          if (this.customize) {
            ...
            resData.validEndTime = null;
          } else {
            resData.validEndTime = this.baseForm.validTime;
            resData.extendTime = null;
          }
          if (this.renewalType == "first") {
            ...
            this.PersonDataList.forEach((item) => {
              resData.cardRenewDetailDtos.push({
                applyObjectType: 0,
                personId: item.bindId,
                endExpireDate: item.expireDate
                  ? item.expireDate
                  : this.getEndExpireDate(),
              });
            });
          } else {
            //  车辆
            this.radioCarList.forEach((item) => {
              resData.cardRenewDetailDtos.push({
                applyObjectType: 1,
                vehicleId: item.baseVehicleDto.managementDtoId, // 车辆管理信息ID
                endExpireDate: item.baseVehicleDto.expireDate,
              });
            });
          }
          if (this.$route.query.id) {
    ...
    getDetails(id) {
      getCardRenewNew(id).then((res) => {
        ...
        if (res.data.renewDetailVo) {
          if (res.data.renewDetailVo.renewVehicleVos.length) {
            this.renewalType = "third";
            this.radioCarList = res.data.renewDetailVo.renewVehicleVos;
            this.radioCarList.forEach((item) => {
              item["baseVehicleDto"] = {};
              item["baseVehicleDto"]["vehicleType"] = item.vehicleType;
              item["baseVehicleDto"]["personName"] = item.personName;
              item["baseVehicleDto"]["personPhone"] = item.personPhone;
              item["baseVehicleDto"]["managementDtoId"] = item.vehicleId;
              item["baseVehicleDto"]["expireDate"] = item.expireDate;
            });
            // baseVehicleDto
          } else {
            this.renewalType = "second";
            this.PersonDataList = res.data.renewDetailVo.renewPersonVos;
            this.PersonDataList.forEach((item) => {
              item["bindId"] = item.personId;
              item["locCardDto"] = {};
              item["locCardDto"]["cardNo"] = item.cardNo;
              item["sysPersonBaseDto"] = {};
              item["sysPersonBaseDto"]["genderName"] = item.genderName;
            });
            console.log(this.PersonDataList);
          }
        } else {
          this.batchList.push(res.data.cardApplyDtos);
          this.$refs.FirstTable.reload();
        }
